...
export default {
    ...
    logout: () => {
        return Promise.all([
            ...
            store.remove(AUTH_CACHE_KEY_NAME),
            store.remove('pay_history'),
            store.remove('reg_merc'),
            store.remove('SWITCH_ACCOUNT'),
            store.remove('$$CHOICE_BUSINESS1'),
            store.remove('$$CHOICE_BUSINESS2')]).catch(error => { throw '缓存清除失败'; })
    },
};
